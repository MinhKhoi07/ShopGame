...
@section('page-title', 'Chỉnh Sửa Sale')
...
<div style="max-width: 800px;">
    <h2 style="color: white; margin-bottom: 30px;">
        ...
        Chỉnh Sửa Sale: {{ $sale->name }}
    </h2>

    <form method="POST" action="{{ route('admin.sales.update', $sale->id) }}" class="admin-form">
        @csrf
        @method('PUT')

        <div class="form-group">
            <label for="name">Tên Sale *</label>
            <input type="text" id="name" name="name" value="{{ old('name', $sale->name) }}" required 
        ...
        <div class="form-group">
            ...
            <textarea id="description" name="description" rows="3" 
                      placeholder="Nhập mô tả về chương trình sale">{{ old('description', $sale->description) }}</textarea>
            @error('description')
                <span class="error">{{ $message }}</span>
            @enderror
        </div>
        ...
        <div class="form-group">
            ...
            <div style="display: flex; gap: 20px; margin-top: 10px;">
                <label style="color: var(--steam-text); cursor: pointer;">
                    <input type="radio" name="sale_type" value="game" 
                           {{ old('sale_type', $sale->game_id ? 'game' : 'category') == 'game' ? 'checked' : '' }} 
                           required onchange="toggleSaleType()">
                    Sale theo Game
                </label>
                <label style="color: var(--steam-text); cursor: pointer;">
                    <input type="radio" name="sale_type" value="category" 
                           {{ old('sale_type', $sale->category_id ? 'category' : 'game') == 'category' ? 'checked' : '' }} 
        ...
        <div class="form-group" id="game_select" style="display: none;">
            ...
            <select id="game_id" name="game_id">
                ...
                @foreach($games as $game)
                    <option value="{{ $game->id }}" {{ old('game_id', $sale->game_id) == $game->id ? 'selected' : '' }}>
        ...
        <div class="form-group">
            ...
            <input type="number" id="discount_percent" name="discount_percent" 
                   value="{{ old('discount_percent', $sale->discount_percent) }}" 
